...
-- Table: subCritter
CREATE TABLE subCritter (    subCritterID int NOT NULL AUTO_INCREMENT,    subCritterLabel varchar(20) NOT NULL COMMENT 'this will be Kith if the subCritter is pooka for example',    critterID int NOT NULL,    critterSubName varchar(20) NOT NULL COMMENT 'Pooka, Gangrel, Get of Fenris ect..',    firstAdvantage blob NOT NULL,    secondAdvantage blob NOT NULL,    flaw varchar(500) NOT NULL,    isDeleted bool NOT NULL DEFAULT 0,    CONSTRAINT subCritter_pk PRIMARY KEY (subCritterID)) COMMENT 'Vampire : clanWerewolf: tribe, auspice, breed Changeling: Kith, seeming, house mage: tradition/convention Wraith ???';
-- Table: user
...
ALTER TABLE Roles ADD CONSTRAINT Roles_user FOREIGN KEY Roles_user (userId)    REFERENCES user (id);
-- Reference: Traits_critter (table: Traits)
ALTER TABLE Traits ADD CONSTRAINT Traits_critter FOREIGN KEY Traits_critter (openToCritterID)    REFERENCES critter (critterID);
-- Reference: Traits_subCritter (table: Traits)
ALTER TABLE Traits ADD CONSTRAINT Traits_subCritter FOREIGN KEY Traits_subCritter (goodAtSubCritterID)    REFERENCES subCritter (subCritterID);
